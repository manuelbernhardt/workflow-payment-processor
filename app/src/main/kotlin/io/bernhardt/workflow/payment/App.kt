...
class App {
    ...
    init {
        ...
        client = WorkflowClient.newInstance(service, clientOpts)

        val factoryOptions = WorkerFactoryOptions.newBuilder().setWorkflowHostLocalPollThreadCount(5).build()

        // create a worker factory that can be used to create workers for specific task queues
        factory = WorkerFactory.newInstance(client, factoryOptions)
